...
export default function LoginEmail() {
    ...
    const { user, signInEmail } = UserAuth()
            ...
            <div className='pt-20 w-72 flex flex-col'>
                <div className='flex flex-col bg-gray-800 border border-gray-700 rounded justify-center items-center px-5 py-5 space-y-4'>
                    ...
                    <div className='flex flex-row justify-center items-center border border-gray-700 rounded-md px-2 py-2 cursor-pointer hover:animate-pulse w-full'>
                        <button className='font-bold text-gray-300 pr-2' onClick={(event) => handleSignIn(event)}>Sign In</button>
                        {loading ? (
                ...
                <button className='justify-center items-center pt-2 text-sm text-gray-500 font-bold hover:scale-110 duration-200'>Forgot password?</button>
                <button className='justify-center items-center pt-2 text-sm text-gray-500 font-bold hover:scale-110 duration-200' onClick={() => navigate('/login')}>Main Login</button>
            </div>
        </div>
  )
}
